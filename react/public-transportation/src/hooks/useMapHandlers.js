...
const useMapHandlers = (position, setPosition, destination, onDestinationSet, setMapCenter, mapCenter) => {
  ...
  // Set the current map center as the starting point
  const handleSetStartPoint = async (customPosition) => {
  ...
  // Set the current map center as the destination point
  const handleSetDestinationPoint = async () => {
    if (mapCenter && Array.isArray(mapCenter) && mapCenter.length === 2) {
  ...
  // Find a route between the current position and destination
